...
+ (NSError *)missingDataErrorWithKey:(const NSString *)key
                             context:(const NSString *)context {
  return [NSError
      errorWithDomain:GLTF2DecodeErrorDomain
                 code:GLTF2ErrorMissingData
             userInfo:@{
               NSLocalizedDescriptionKey : [NSString
                   stringWithFormat:@"Key '%@' not found in %@.", key, context]
             }];
}

+ (NSError *)invalidFormatErrorWithKey:(const NSString *)key
                               objName:(const NSString *)context {
  return [NSError
      errorWithDomain:GLTF2DecodeErrorDomain
                 code:GLTF2ErrorInvalidFormat
             userInfo:@{
               NSLocalizedDescriptionKey : [NSString
                   stringWithFormat:@"Invalid value format for key '%@' in %@.",
                                    key, context]
             }];
}

//+ (NSInteger)getUInt:(const NSDictionary *)jsonDict
...
//    return nil;
//
//  NSInteger componentType = [GLTFDecoder getUInt:jsonDict
//                                              key:@"componentType"
//                                         required:YES
//                                          objName:objName
//                                            error:error];
...
//    return nil;
//  }
//  NSInteger modeValue = modeString ? GLTFPrimitiveModeFromString(modeString)
